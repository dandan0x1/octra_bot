...
async def multi():
    ...
    if len(wallets) != len(proxies):
        print(f"钱包数量（{len(wallets)}）和代理数量（{len(proxies)}）不一致！请检查 config/all_outputs.json 和 config/proxy.txt")
        return
    amount = float(input("请输入每个地址要发送的金额: ").strip())
    batch_size = 10
    max_proxy_retries = 3

    for sender, proxy in zip(wallets, proxies):
        print(f"\n--- 当前发送方: {sender['address']} 使用代理: {proxy} ---")
        candidates = [w['address'] for w in wallets if w['address'] != sender['address']]
            ...
            try:
                ...
                if n is None or not b or b < tot:
                    ...
                    if retry == max_proxy_retries:
                        print(f"[代理{proxy}] 连续{max_proxy_retries}次失败，跳过该发送方。")
                    continue
                # 发送
                ...
                if retry == max_proxy_retries:
                    print(f"[代理{proxy}] 连续{max_proxy_retries}次异常，跳过该发送方。")
